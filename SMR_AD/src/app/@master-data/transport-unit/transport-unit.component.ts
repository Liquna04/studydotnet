...
@Component({
  selector: 'app-transport-unit',
  standalone: true,
  imports: [ShareModule],
  templateUrl: './transport-unit.component.html',
  styleUrl: './transport-unit.component.scss',
})
export class TransportUnitComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private _service: TransportUnitService,
    ...
    this.globalService.setBreadcrumb([
      { name: 'Danh sách đơn vị vận tải', path: 'master-data/transport-unit' },
    ]);
  ...
  updateTableScrollHeight() {
    if (window.innerWidth <= 768) {
      this.tableScrollY = null; // Ensure tableScrollY is set to null
